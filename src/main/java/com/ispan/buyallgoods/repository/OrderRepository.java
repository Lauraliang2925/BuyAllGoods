...
public interface OrderRepository extends JpaRepository<OrderBean, Integer> {
	...
	public List<Object[]> getAllOrdersWhereMemberID(@Param("members_id")Integer members_id);
...
//	
	@Query(nativeQuery = true,
			value = "select * from orders where order_notes like CONCAT('%', :order_notes, '%')")
	public List<Object[]> searchOrderByNotes2(@Param("order_notes")String order_notes);
	
	@Query("from OrderBean where order_notes like %:order_notes% ")
	List<OrderBean> searchOrderByNotes(@Param("order_notes")String order_notes);

  ...
  methods: {
    findAllOrdersWhereMember() {
      ...
      if (vm.MemberId != null && vm.RoleId === "3") {
        ...
        axios
          .post(contextPath + "/api/page/orders/members/" + vm.MemberId)
          .then((response) => {
        ...
        axios
          .get(contextPath + "/api/page/orders/find")
          .then((response) => {
            // console.log(response.data)
            vm.OrdersWhereMemberData = response.data;
            vm.backupOrdersData = response.data;
    ...
    updateByOrderStatus() {
      ...
      if (vm.roleId === "3") {
        if (vm.selectedStatus !== "所有訂單") {
          vm.OrdersWhereMemberData = vm.backupOrdersData.filter(
            (item) => item.order_status === vm.selectedStatus
          );
        ...
        if (vm.selectedStatus !== "所有訂單") {
          vm.OrdersWhereMemberData = vm.backupOrdersData.filter(
            (item) => item.orderStatus === vm.selectedStatus
          );
          if (vm.OrdersWhereMemberData.length === 0) {
            vm.isShowTable = false;
          }
    ...
    selectOrderId(orderId) {
      ...
      axios
        .post(contextPath + "/api/page/orders/" + orderId)
        .then((response) => {
          // console.log(response.data)
    ...
    searchOrderNotes() {
      ...
      if (vm.roleId === "3") {
        axios
          .post(
            contextPath +
              "/api/page/orders/searchByNotes2/" +
              searchInputValue +
              "/" +
              vm.membersId
          )
        ...
        axios
          .post(
            contextPath +
              "/api/page/orders/searchByNotesAll/" +
              searchInputValue
          )
          .then((response) => {
            // console.log(response.data)
        ...
        callback: (result) => {
          if (result) {
            axios
              .put(
                contextPath +
                  "/api/page/orders/modifyByStatusReturn/" +
                  vm.order_id
              )
              .then((response) => {
        ...
        callback: (result) => {
          if (result) {
            ...
            axios
              .put(
                contextPath +
                  "/api/page/orders/modifyByStatusCancel/" +
                  vm.order_id
              )
              .then((response) => {
                const notyf = new Notyf({
    ...
    getInnerJoinProductAndOrderByMemberIdAndOrderId() {
      ...
      vm.disableButton = false;
      vm.cancel = false;
      vm.isShowReviewButton = false;

      axios
        .post(
          contextPath +
            "/api/page/orders/detail/findInnerJoin/" +
            vm.members_id +
            "/" +
            vm.order_id
        )

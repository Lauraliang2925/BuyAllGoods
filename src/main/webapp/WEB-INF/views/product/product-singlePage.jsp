	...
	<div class="container" id="app">
		...
		<div class="d-flex">
          ...
          <use xlink:href="#bootstrap"></use>
				...
				<hr />
				<ul class="nav nav-pills  flex-column mb-3 align-items-center fs-5"
					v-for="category in categories" :key="category.categoriesId">
          ...
          <title>{{name}}</title>
          <image :xlink:href="contextPath + '/pic/product/'+name+'.jpg'"
			...
			<div class="bg-light card-body py-5 container">
				...
				<br />
				<div class="d-flex container fs-9 text-danger"
					v-if="isShowDiscountDate">
					<div>優惠倒數：</div>
					<div id="countdown"></div>
				</div>
				<div class="container fs-5" v-if="isShowDiscountDate">優惠期限:
					{{discountStartDate}} ~ {{discountEndDate}}</div>
				<br />
				<!-- 商品敘述/注意事項起始 -->
				...
				<!-- 商品價格/數量/優惠券起始 -->
				<div class="container d-flex justify-content-around ">
					<div class="fs-4 fw-bold " v-if="discount>=1">價格:$
						{{sellingPrice}}</div>
					...
					<div
						...
						<div class="fs-4 fw-bold text-danger">優惠價格:$
							{{Math.floor(sellingPrice*discount)}}</div>
					</div>
				...
				<div
					...
					<div class="container ">
						數量<input type="number" min="0" style="width: 50px; height: 30px"
							v-model="quantity" />
					</div>
				</div>
				...
				<div class="container gap-2 py-3 d-flex justify-content-around">
					<button class="btn btn-sm btn-outline-primary" type="button"
						@click="addFavorites(productsId)">加入收藏清單</button>
					<button class="btn btn-sm btn-outline-primary" type="button"
						@click="addShoppingcarts(productsId)">加入購物車</button>
				</div>
...
<script type="text/javascript"
	src="<c:url value='/js/product/product-singlePage.js'></c:url>"></script>
</html>

        ...
        <div class="row mb-3">
            <div class="col-6">
                <div class="input-group mb-3">
                    <!-- <input type="text" class="form-control" placeholder="搜尋所有訂單" v-model="searchInput" @blur="searchOrderNotes" /> -->
            ...
            <div class="col-3">
                <select class="form-select" aria-label="Default select example" v-model="selectedTimeRange" @change="updateTimeRange">
            ...
            <div class="container" v-show="isShowCard" >
                <div class="row mt-3" v-if="getInnerJoinData.length > 0">
                    <div class="col-auto">
                        <p class="text-start text-lowercase text-success fw-bold">{{getInnerJoinData[0].order_status}}</p>
                    </div>
                    <div class="col-3">
                        <p class="text-start text-lowercase fw-bold">訂單成立日期：{{getInnerJoinData[0].placed}}</p>
                    </div>
                    <div class="col-2">
                        <p class="text-start text-lowercase fw-bold">總金額：<i class="bi bi-currency-dollar"></i> {{getInnerJoinData[0].total_amount}}</p>
                    </div>
                </div>
                <div class="card">
                    <table class="table table-borderless" v-for="(item,index) in getInnerJoinData" :key="item.order_detail_id">
                        <tbody>
                            <tr>
                                <td>
                                    <div class="d-flex mb-3 h4">
                                        <div class="d-flex flex-row align-items-center"><img class="rounded float-start"
                                                    src="https://memeprod.ap-south-1.linodeobjects.com/user-template-thumbnail/0cd466686070c9edf2c01fad47919f56.jpg"></div>
                                        <div class="d-flex flex-column">
                                        <div class="d-flex flex-row justify-content-start">商品名稱：{{item.name}}</div>
                                        <br>
                                        <div class="d-flex flex-row justify-content-start">數量 : {{item.quantity}}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column ">
                                        <button class="btn btn-outline-primary mb-2 mt-3 btn-lg" v-if="index === 0"   @click="redirectToOrderDetail(item.order_id)">
                                        查看訂單細節
                                        </button>
                                        <!-- <button class="btn btn-outline-primary mb-2 btn-lg" v-if="index === 0" @click="addToShoppingCart(item)">重新購買</button> -->
                                        <button class="btn btn-outline-dark mb-2 btn-lg" @click="modifyOrderStatusReturn" v-if="index === 0" v-show="!disableButton">退貨</button>
                                        <button class="btn btn btn-dark mb-2 btn-lg" v-if="index === 0" disabled v-show="disableButton">退貨</button>
                                        <button class="btn btn-outline-dark mb-2 btn-lg" @click="modifyOrderStatusCancel" v-if="index === 0" v-show="!cancel">取消訂單</button>
                                        <button class="btn btn btn-dark mb-2 btn-lg" v-if="index === 0" v-show="cancel" disabled>取消訂單</button>
                        ...
                        <tbody>
                            ...
                            <tr class="align-middle" v-for="item in OrdersWhereMemberData" :key="item.order_id"
                                ...
                                <td><button type="button" class="btn btn-outline-secondary btn-lg"
                                        @click="selectOrderId(item.order_id)"><i class="bi bi-search"></i> 查看</button>
                                </td>
                            </tr>
                            <!-- 一般 -->
                            <!-- 搜尋 -->
